...
const { API_TIMEOUT_ENTSOE } = require('../main.js');
...
async function getDataEnergyCharts(adapter, tomorrow, country) {
    ...
    country = country.toUpperCase();
    adapter.log.info(`getDataEnergyCharts: country selected is ${country}, date ${dateString}`);
    country = country == 'DE' ? 'DE-LU' : country;
    adapter.log.info(`getDataEnergyCharts: country mapped is ${country}, date ${dateString}`);
